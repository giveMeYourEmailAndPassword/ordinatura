      ...
      <TableFrame>
        {visibleRows.length ? (
          <table className="min-w-[1080px] w-full border-collapse">
            <thead><tr>{['№', 'ФИО', 'Год', 'Специальность', 'Кафедра', 'Источник', 'Телефон', ''].map((heading, index) => <th key={`${heading}-${index}`} className={heading === 'Телефон' ? `${tableHeadClass} pl-8` : tableHeadClass}>{heading}</th>)}</tr></thead>
            <tbody>{visibleRows.map((row, index) => (
              <tr key={row.id} className="hover:bg-stone-50">
                ...
                <td className={`${tableCellClass} pl-8`}>{row.phone || '—'}</td>
