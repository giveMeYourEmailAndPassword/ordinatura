...
function scopeRows() {
  ...
  return state.rows.filter(row => {
    ...
    return found && row.program === state.section && (state.department === 'all' || row.department === state.department);
  });
}
...
function render() {
  ...
  const t = totals();
  const sectionRows=state.rows.filter(row=>row.program===state.section);
  const departments = [...new Set(sectionRows.map(r => r.department).filter(Boolean))].sort((a,b) => a.localeCompare(b,'ru'));
  const personWord=state.section==='Интернатура'?'интернов':'ординаторов';
  const personTitle=state.section==='Интернатура'?'Интерн':'Ординатор';
  document.querySelector('#app').innerHTML = `
    <div class="shell">
      <aside class="sidebar">
        ...
        <div class="nav"><button class="${state.section==='Ординатура'?'active':''}" data-section="Ординатура">▦ &nbsp; Распределение ставок Ординатура</button><button class="${state.section==='Интернатура'?'active':''}" data-section="Интернатура">▦ &nbsp; Распределение ставок Интернатура</button><button id="navImport">⇧ &nbsp; Импорт из Excel</button></div>
      ...
      <main class="main">
        <header class="topbar"><div><h1>Распределение ставок — ${state.section}</h1><p class="subtitle">2026–2027 учебный год · ППС и УВП по ФПМО</p></div>
        ...
        <section class="cards staffing-cards">
          <div class="card total-card"><div class="label">Общий итог</div><div class="value">${t.people}</div><div class="hint">${personWord} в выбранном списке</div></div>
          <div class="card rate-card budget-card"><div><div class="label">Бюджет</div><div class="count">${t.budget} ${personWord}</div></div><div class="rate"><span>Ставка</span><strong>${fmtRate(t.budgetRate)}</strong><small>${t.budget} ÷ 4</small></div></div>
          <div class="card rate-card contract-card"><div><div class="label">Контракт + СНГ</div><div class="count">${t.contract} ${personWord}</div></div><div class="rate"><span>Ставка</span><strong>${fmtRate(t.contractRate)}</strong><small>${t.contract} ÷ 4</small></div></div>
        </section>
        ...
        <div class="table-wrap">${rows.length ? `<table><thead><tr><th>№</th><th>${personTitle}</th><th>Специальность</th><th>Кафедра</th><th>Программа</th><th>Источник</th><th>Телефон</th><th></th></tr></thead><tbody>${rows.map((r,i)=>rowHtml(r,i)).join('')}</tbody></table>` : `<div class="empty"><strong>Записей пока нет</strong>Добавьте человека вручную или загрузите исходную таблицу Excel для раздела «${state.section}».</div>`}</div>
    </div>`;
  bind();
}

function rowHtml(row, index) {
  return `<tr><td class="muted">${index+1}</td><td><div class="person">${esc(row.name)}</div>${row.degree ? `<div class="muted">${esc(row.degree)}</div>`:''}</td><td>${esc(row.specialty || row.position || '—')}</td><td>${esc(row.department || '—')}</td><td>${esc(row.program || '—')}</td><td><span class="badge ${row.funding}">${FUNDING[row.funding] || 'Не указан'}</span></td><td>${esc(row.phone || '—')}</td><td><div class="row-actions"><button class="icon-btn edit" data-id="${row.id}" title="Изменить">✎</button><button class="icon-btn delete" data-id="${row.id}" title="Удалить">×</button></div></td></tr>`;
}
function bind() {
  ...
  document.querySelectorAll('[data-section]').forEach(button => button.onclick=()=>{state.section=button.dataset.section;state.department='all';state.funding='all';state.search='';render();});
...
function openForm(row = {}) {
  ...
  modal.innerHTML=`<div class="modal"><div class="modal-head"><h2>${row.id?'Редактировать запись':'Новая ставка'}</h2><button class="close">×</button></div><form class="form" id="personForm">
    <div class="import-note">Одна строка — одна ставка. Для одного сотрудника можно добавить несколько строк: например, 0,5 бюджета ординатуры и 0,25 контракта интернатуры.</div>
    <div class="grid">
      ...
      ${field('department','Кафедра',row.department,'text',false)}${selectField('program','Программа',row.program || state.section,[['Ординатура','Ординатура'],['Интернатура','Интернатура']])}
  ...
  modal.querySelector('form').onsubmit=e=>{e.preventDefault(); const data=Object.fromEntries(new FormData(e.target)); const saved={...row,...data,id:row.id||uid(),rate:numeric(data.rate)}; if(row.id) state.rows=state.rows.map(r=>r.id===row.id?saved:r); else state.rows.push(saved); saveRows(); close(); render(); showToast('Запись сохранена');};
...
async function importFiles(event) {
  ...
  for(const file of files) {
    ...
    for(const sheetName of workbook.SheetNames) imported.push(...parseSheet(workbook.Sheets[sheetName],targetSection,sourceNames));
  }
  state.rows=state.rows.filter(row=>row.program!==targetSection || !sourceNames.has(row.name.toLowerCase()));
  ...
  state.rows.push(...imported); saveRows(); render(); showToast(`${targetSection}: загружено действующих записей — ${imported.length}`); event.target.value='';
}
function parseSheet(sheet,program,sourceNames) {
  ...
  // Раздел определяется местом загрузки, а не названием или содержимым файла.
  const output=[];
  let accepting=true;
...
function exportExcel() {
  ...
  const personGenitive=state.section==='Интернатура'?'интерна':'ординатора';
  const personPlural=state.section==='Интернатура'?'интернов':'ординаторов';
  const data=ordered.map((r,i)=>({'№':i+1,[`Фамилия, имя ${personGenitive}`]:r.name,'Кафедра':r.department,'Специальность':r.specialty,'Программа':r.program,'Источник':FUNDING[r.funding]||'','Телефон':r.phone}));
  const summary=[
    [`РАСЧЕТ ШТАТНЫХ СТАВОК: ${state.section.toUpperCase()} · 2026–2027 УЧЕБНЫЙ ГОД`],
    ['Фильтр кафедры',state.department==='all'?'Все кафедры':state.department],
    ['Раздел',state.section],
    [],
    [`Общий итог ${personPlural}`,t.people],
    ['Бюджет',t.budget],
    ['Контракт + СНГ',t.contract],
    [],
    ['Источник',`Количество ${personPlural}`,'Формула','Итоговая ставка'],
  ...
  ws['!cols']=[{wch:5},{wch:38},{wch:34},{wch:30},{wch:15},{wch:18},{wch:16}]; ws['!autofilter']={ref:`A1:G${data.length+1}`};
  sum['!cols']=[{wch:25},{wch:24},{wch:18},{wch:20}];
  XLSX.utils.book_append_sheet(wb,ws,`Список — ${state.section}`); XLSX.utils.book_append_sheet(wb,sum,'Расчет ставок');
  XLSX.writeFile(wb,`Штатное_расписание_${state.section}_2026-2027_${new Date().toISOString().slice(0,10)}.xlsx`); showToast('Штатное расписание сформировано');
}
